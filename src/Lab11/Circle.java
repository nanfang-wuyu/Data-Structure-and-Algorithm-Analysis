...
public class Circle extends Shape implements Comparable<Circle>, ColorScheme.ColorDraw {
    @Override
    ...
    public boolean isInBoundary() {
        if (-1 * getScreenSize() > x - this.radius
                || getScreenSize() < x + this.radius) {
            return false;
        }
        if (-1 * getScreenSize() > y - this.radius
                || getScreenSize() < y + this.radius) {
            return false;
        }
        return true;
    }
