/*package dsaalab9;

import java.util.*;


public class dsaa20190091new {
    static public class treeNode{
        int key;
        treeNode leftChild;
        treeNode rightChild;
        treeNode parent;
        public treeNode(int key){this.key = key;}
    }
    public static class Vertex{
        ArrayList<Edge>list = new ArrayList<>();
        int key,color;
        Vertex parant;
        public Vertex(int key){
            this.key = key;
            this.color = 0;//whi
            this.parant = null;
        }
    }

    public static class Edge{
        int target,weight;
        public Edge(int target, int weight){
            this.target = target;
            this.weight = weight;
        }
    }

    public static void insert(ArrayList<treeNode>list, int item){
        list.add(new treeNode(item));//step1
        treeNode u = list.get(list.size()-1);
        if(list.size()-1!=1){
            treeNode p = list.get((list.size()-1)/2);
            u.parent = p;
            if(p.leftChild==null) p.leftChild = u;
            else p.rightChild = u;
        }
        //step2
        while(true) {
            if (u.parent==null) return;//step3
            if (u.key > u.parent.key) return;//step4
            swapkey(u, u.parent);//step5.1
            u = u.parent;//step5.2
        }
    }
    public static int deleteMin(ArrayList<treeNode>list){
        treeNode root = list.get(1);
        treeNode rightmost = list.get(list.size()-1);
        int min = root.key;
        root.key = rightmost.key;//step1,2
        if(list.size()!=2) {
            if ((list.size() - 1) % 2 == 0) rightmost.parent.leftChild = null;
            else rightmost.parent.rightChild = null;
        }
        list.remove(list.size()-1);//step3
        treeNode u = root;//step4
        while(true) {
            if (u.leftChild == null && u.rightChild == null){
                return min;//step5
            }
            if (u.rightChild == null){
                if(u == keySmallerNode(u,u.leftChild))return min;
            }
            if (u.rightChild != null){
                if(u == keySmallerNode(u,u.leftChild) &&
                        u == keySmallerNode(u,u.rightChild))return min;
            }
            treeNode v = u.leftChild;
            if(u.rightChild!=null)v = keySmallerNode(u.leftChild,u.rightChild);
            swapkey(u,v);
            u = v;//step7
        }
    }
    public static void swapkey(treeNode a, treeNode b){
        int temp = a.key;
        a.key = b.key;
        b.key = temp;
    }
    public static treeNode keySmallerNode(treeNode a, treeNode b){
        if(a.key<b.key)return a;
        else return b;
    }

    static Vertex [] v;
    static int[] M, dist;
    static int n, m, start, target, cnt, front, rear;
    static int max = 2147483647;
    static Scanner in = new Scanner(System.in);

    public static void main(String[] args) {
        dsaa20190091new a = new dsaa20190091new();
        a.input();
        a.print();
    }

    public void input(){
        n = in.nextInt();
        m = in.nextInt();
        cnt = 0;
        front = 1;
        rear = 1;
        v = new Vertex[n+1];
        dist = new int[n+1];
        M = new int[n+5];
        for(int i = 1;i<n+1;i++){
            v[i] = new Vertex(i);
            dist[i] = max;
        }
        for(int i = 1;i<m+1;i++){
            int a = in.nextInt();
            int b = in.nextInt();
            int w = in.nextInt();
            v[a].list.add(new Edge(b,w));
            v[b].list.add(new Edge(a,w));
        }
        start = in.nextInt();
        target = in.nextInt();
        v[start].color = 1;
        dist[start] = 0;
        Dijkstra(start);
    }
    public void Dijkstra(int start){
        int min = start;
        for(int j = 1;j<n+1;j++) {
            for (int i = 0; i < v[min].list.size(); i++) {
                int t = v[min].list.get(i).target;
                updateDist(min, t, v[min].list.get(i).weight);
            }
            min = M[1];
            deleteMin(M);
        }
    }

    public void updateDist(int u, int t, int weight){
       if(dist[t]>dist[u]+weight) {
           dist[t] = dist[u] + weight;

       }
    }

    public void print(){
        if(dist[target]!=max)System.out.println(dist[target]);
        else System.out.println(-1);
    }


}*/
/*public int findmin(){
        int mindist = max;
        int min = -1;
        for(int i = 1;i<n+1;i++){
            if(v[i].color==0)
            if(dist[i]<mindist){
                mindist = dist[i];
                min = i;
            }
        }
        if(min!=-1)v[min].color = 1;
        return min;
    }*/